...
import {
	TIME_EXPIRATION_CONFIRM_EMAIL,
	TIME_EXPIRATION_FORGOT_PASSWORD,
...
export class EmailController {
	...
	async forgotPassword(request: Request, response: Response) {
		...
		const timeExpirationPassword = Number(
			user.security.passwordResetExpiration
		);

		if (
			timeExpirationPassword > 0 &&
			timeExpirationPassword < currentTime &&
			tokenPasswordResend.length
		) {
			return response.status(STATUSCODE.SERVICE_UNAVAILABLE).send({
				message:
					"Um token ja foi enviado ao seu email, utilize-o para recuperar sua senha.",
			});
		}
	...
	async recoverPassword(request: Request, response: Response) {
		...
		const currentTimeOfExpirationPasswordReset = Number(
			user.security.passwordResetExpiration
		);
