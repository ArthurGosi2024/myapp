		...
		if (findUser) {
			...
			if (!confirmEmailAccount) {
				return response.status(STATUSCODE.NOT_FOUND).send({
					message:
						"Não identificamos a confirmação de email, identifique seu email.",
				});
			}
			...
			if (isAccountLocked) {
				...
				return response.status(STATUSCODE.CONFLICT).send({
					message: `Tentativas bloqueadas. Tente novamente no dia ${releaseDate.toLocaleString(
						"pt-BR",
						{
							year: "numeric",
							month: "long",
			...
			if (passwordUser !== password) {
				if (
					MAXIMUM_LOGIN_ATTEMPTS - findUser.security.failedLoginAttempts ===
					0
				) {
				...
				return response.status(STATUSCODE.CONFLICT).send({
					message: `Senha incorreta. Você ainda possue mais ${
						MAXIMUM_LOGIN_ATTEMPTS - findUser.security.failedLoginAttempts
					} tentativas.`,
				});
			}
